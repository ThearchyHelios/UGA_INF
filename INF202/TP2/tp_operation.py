...
def getPredefinie33():
    print(
        "TODO: modifier la relation pour qu'elle aille les bonnes dimensions: 3x3"
    )
    return [[1, 0, 0], [1, 1, 1], [0, 1, 1]]
    # return [[1, ], [1, 1, 1, 1], [1, 1]]


def getPredefinie35():
    print(
        "TODO: modifier la relation pour qu'elle aille les bonnes dimensions: 3x5"
    )
    return [[1, 0, 0, 1, 0], [1, 1, 1, 0, 0], [0, 1, 1, 1, 0]]
...
def getPredefinie53():
    ...
    print(
        "TODO: modifier la relation pour qu'elle soit l'inverse de getPredefinie35"
    )
    return [[1, 1, 0], [0, 1, 1], [0, 1, 1], [1, 0, 1], [0, 0, 0]]
...
def affiche(R):
    ...
    print("\nAffichage: ")
    print(
        "/!\ Selon la version de Python, l'affichage ne fonctionne pas de la meme facon.\n Commenter les 'print' utilises et decommenter les autres. "
    )
...
def verifier_taille(R, n, m):
    ...
    if len(R) != n:
        print(
            "Probleme de taille: {} attendu, mais relation a {} lignes".format(
                n, len(R)))
        return False
    ...
    for i in range(n):
        if len(R[i]) != m:
            print(
                "Probleme de taille dans la ligne {}: {} attendu, mais la relation a {} colonnes"
                    .format(i, m, len(R[i])))
            return False
...
def union(R, S):
    ...
    '''
    RuS = []
    if not verifier_taille(R, len(S), len(S[0])):
        return []
    else:
        for i in range(len(R)):
            RuS.append([])
            for j in range(len(R[i])):
                RuS[i].append(R[i][j] or S[i][j])
...
def ex_union():
    ...
    if sont_egales(RR1_U_RR2, RR1):
        print("Test Union egal OK")
    else:
        print("Test Union egal FAIL")

    if est_incluse(RR1, RR1_U_RR2) and est_incluse(RR2, RR1_U_RR2):
        print("Test Union incluse OK")
    else:
        print("Test Union incluse FAIL")
...
def ex_inverse():
    ...
    print(
        "TODO: Creer des examples en utilisant que l'inverse est distributive avec l'union: (A union B)^-1  =  A^-1 union B^-1"
    )

    print(
        "TODO: Creer des examples en utilisant que l'inverse est distributive avec l'intersection: (A intersection B)^-1  =  A^-1 intersection B^-1"
    )
...
def ex_composition():
    ...
    print(
        "TODO: Montrer que la composition N'EST PAS distributive avec l'intersection"
    )

    print(
        "TODO: Montrer que la composition d'une intersection est incluse dans l'intersection de compositions"
    )
